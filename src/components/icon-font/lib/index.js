...
const {CSS_PARSE_REGEX, FONT_TYPES, TEMPLATES, OPTIONAL_PARAMS, DEFAULT_OPTIONS} = require("./constants");
...
function getGeneratorConfig(options) {
   ...
   const config = {
      ...
      templateOptions: {
         ...
         htmlCssRelativePath: path.relative(path.dirname(getResolvedPath(options, "html")), getResolvedPath(options, "css"))
      }
   };
...
async function generateJson(options, generatorResult) {
   const jsonPath = options.jsonPath || `${path.join(options.outputDir, "/" + options.fontName)}.json`;
...
async function validateOptions(options) {
   ...
   if (options.codepoints) {
      ...
      if (!codepointsStat.isFile() || path.extname(options.codepoints) !== ".json") {
         throw new ValidationError(["Codepoints file must be JSON", `${options.codepoints} is not a valid file.`].join(" "));
      }
   }
}
